...
| `CompanionManager+PendingAction.swift` | ~650 | Phase D (U11). Extension on CompanionManager wiring the pending-action queue lifecycle: act-mode UserDefaults flag, `processActionTagsAndEnqueue` (response pipeline integration), kill-switch observation (`bindActModeKillSwitchObservation`), confirmation panel presentation/outcome handling, walkthrough integration, highlight annotation lifecycle. Uses `PendingActionStateStorage` (associated object) for stored state. |
