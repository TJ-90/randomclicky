...
macOS menu bar companion app. Lives entirely in the macOS status bar (no dock icon, no main window). Clicking the menu bar icon opens a custom floating panel with companion voice controls. Uses push-to-talk (ctrl+option) to capture voice input, transcribes it via AssemblyAI streaming, and sends the transcript + a screenshot of the user's screen to the configured AI provider. The provider responds with text (streamed via SSE on the Claude path) and voice (ElevenLabs TTS or local macOS speech). A blue cursor overlay can fly to and point at UI elements the provider references on any connected monitor.

Remote API keys live on a Cloudflare Worker proxy or local user config — nothing sensitive ships in the app.
...
- **AI Chat**: Claude (Sonnet 4.6 default, Opus 4.6 optional) via Cloudflare Worker proxy with SSE streaming; optional local `llm.json` routing to OpenRouter, Ollama, or Codex CLI (`codex exec`)
- **Speech-to-Text**: AssemblyAI real-time streaming (`u3-rt-pro` model) via websocket, with OpenAI and Apple Speech as fallbacks
- **Text-to-Speech**: ElevenLabs (`eleven_flash_v2_5` model) via Cloudflare Worker proxy
- **Screen Capture**: ScreenCaptureKit (macOS 14.2+), multi-monitor support
- **Voice Input**: Push-to-talk via `AVAudioEngine` + pluggable transcription-provider layer. System-wide keyboard shortcut via listen-only CGEvent tap.
- **Element Pointing**: The AI provider embeds `[POINT:x,y:label:screenN]` tags in responses. The overlay parses these, maps coordinates to the correct monitor, and animates the blue cursor along a bezier arc to the target.
...
**Act-Mode Conventions (Phase D)**: Act mode is OFF by default (R14) and toggled from the panel. The system prompt's CLICK/TYPE grammar paragraph is appended only when act mode is on — `CompanionManager.companionVoiceResponseSystemPrompt(actModeEnabled:)` is a pure static function checked at request time. Actions use element IDs only (no pixel-coordinate form): `[CLICK:E<id>:description]` / `[TYPE:E<id>:text:description]`. TYPE payloads must never contain newlines or control characters, and must NEVER appear in any analytics payload — enforced at the `ClickyAnalytics.buildActionEventProperties` signature level (no typed-text parameter exists). Every action requires explicit per-action user confirmation before execution. Direct focused-field commands beginning with "type " or "enter " may bypass screenshot/provider routing only when act mode is on and macOS reports a focused editable AX element; the generated local TYPE action still uses the same confirmation panel and execution safety chain.
...
| `ClaudeAPI.swift` | ~347 | Claude vision API client with streaming (SSE) and non-streaming modes. TLS warmup optimization, image MIME detection, conversation history support. Accepts an optional `supplementalContextText` parameter to inject AX element inventory and visible text context as a text block after images. |
| `CodexAPI.swift` | ~385 | Local Codex CLI bridge. Writes current screenshots to a temp directory, invokes `codex exec` in ephemeral read-only mode with screenshots attached, reads the final response via `-o`, and returns plain Clicky response text/tags for the existing TTS, overlay, pointing, walkthrough, and act-mode parsers. Supports saved Codex auth or `--oss --local-provider ollama` via `llm.json`. |
| `LLMProviderConfiguration.swift` | ~210 | Runtime provider config loader for `~/Library/Application Support/Clicky/llm.json`. Supports OpenRouter, Ollama, and Codex CLI routing plus local voice output and Codex-specific options like executable path, working directory, sandbox, timeout, OSS mode, and local provider. |
| `OpenAIAPI.swift` | ~142 | OpenAI GPT vision API client. |
| `ElevenLabsTTSClient.swift` | ~81 | ElevenLabs TTS client. Sends text to the Worker proxy, plays back audio via `AVAudioPlayer`. Exposes `isPlaying` for transient cursor scheduling. |
| `AccessibilityElementInventoryService.swift` | ~1469 | Bounded, non-blocking AX walk that enumerates actionable elements and visible readable text in the frontmost window on a dedicated serial queue. Returns `AccessibilityElementInventory` with per-element CG/AppKit frames plus text-only context items that are not actionable IDs. Also appends the focused editable element to normal inventories when needed, and exposes a focused editable-element capture fast path for local act-mode typing commands. |
...
| `ActionTagParser.swift` | ~441 | Phase D (U11). Pure static scanning parser for CLICK and TYPE action tags emitted by Claude. Element-ID targets only — pixel-coordinate forms are rejected at parse time as the act-mode safety floor. Returns `ActionTagParseResult` with parsed actions in document order and all action tags stripped from the spoken text. Also includes `LocalActModeCommandParser` for narrow "type " / "enter " focused-field commands. |
...
| `CompanionManager+PendingAction.swift` | ~871 | Phase D (U11/U12). Extension on CompanionManager wiring the pending-action queue lifecycle: act-mode UserDefaults flag, local focused-field type command handling, `processActionTagsAndEnqueue` (response pipeline integration), kill-switch observation (`bindActModeKillSwitchObservation`), confirmation panel presentation/outcome handling, walkthrough integration, highlight annotation lifecycle. Uses `PendingActionStateStorage` (associated object) for stored state. U12: analytics calls at all four action lifecycle points (proposed/confirmed/cancelled/failed); failure cases broken out by kind for granular `failure_reason` tracking. |
