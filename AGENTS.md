...
| `AccessibilityElementInventoryService.swift` | ~1408 | Bounded, non-blocking AX walk that enumerates actionable elements and visible readable text in the frontmost window on a dedicated serial queue. Returns `AccessibilityElementInventory` with per-element CG/AppKit frames plus text-only context items that are not actionable IDs. Also exposes a focused editable-element capture fast path for local act-mode typing commands. |
