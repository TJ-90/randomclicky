...
| `AccessibilityElementInventoryService.swift` | ~1251 | Bounded, non-blocking AX walk that enumerates actionable elements in the frontmost window on a dedicated serial queue. Returns `AccessibilityElementInventory` with per-element CG and AppKit frames. Also exposes a focused editable-element capture fast path for local act-mode typing commands. |
